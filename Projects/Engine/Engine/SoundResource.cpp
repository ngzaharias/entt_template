...
bool audio::SoundLoader::Import(const str::Path& inputPath, const str::Path& outputPath) const
{
	...
	resource_type.SetString("sound", document.GetAllocator());
...
std::shared_ptr<audio::SoundResource> audio::SoundLoader::load(const core::ResourceEntry& entry) const
{
	rapidjson::Document document;
	...
	resource->m_SoundBuffer.loadFromMemory(binaryData.m_Data, binaryData.m_Size);
