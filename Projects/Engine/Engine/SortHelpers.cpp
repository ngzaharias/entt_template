...
namespace natsort
{
	...
	static int Compare_Right(const std::string_view& a, const std::string_view& b)
	{
		int bias = 0;
	...
	static int Compare_Left(const std::string_view& a, const std::string_view& b)
	{
	...
	static int Compare(const std::string_view& a, const std::string_view& b, int fold_case)
	{
		int ai, bi;
		char ca, cb;
		...
		while (1)
		{
			...
			if (IsDigit(ca) && IsDigit(cb))
			{
				...
				const std::string_view aa = a.substr(ai, a.size() - 1);
				const std::string_view bb = b.substr(bi, b.size() - 1);
	...
	int Compare_NoCase(const std::string_view& a, const std::string_view& b)
	{
